...
-- 테스트 값
INSERT INTO users VALUES (13,'kimtest','seoul',99.02,2,'000915-4');
INSERT INTO users VALUES (14,'Parktest','suwon',22.62,3,'010722-3');
SELECT * FROM users;
...
SELECT name 이름, jumin 주민번호,
    CASE WHEN TO_NUMBER(SUBSTR(jumin,3,2)) >= TO_NUMBER(TO_CHAR(SYSDATE,'MM'))
              AND TO_NUMBER(SUBSTR(jumin,5,2)) >= TO_NUMBER(TO_CHAR(SYSDATE,'DD'))
         THEN CASE WHEN TO_NUMBER(SUBSTR(jumin,1,2)) <= TO_NUMBER(TO_CHAR(SYSDATE,'YY'))
                   THEN TO_NUMBER(TO_CHAR(SYSDATE,'YY')) - TO_NUMBER(SUBSTR(jumin,1,2)) - 1
                   ELSE (100+TO_NUMBER(TO_CHAR(SYSDATE,'YY'))) - TO_NUMBER(SUBSTR(jumin,1,2)) - 1
                   END
         ELSE CASE WHEN TO_NUMBER(SUBSTR(jumin,1,2)) <= TO_NUMBER(TO_CHAR(SYSDATE,'YY'))
                   THEN TO_NUMBER(TO_CHAR(SYSDATE,'YY')) - TO_NUMBER(SUBSTR(jumin,1,2))
                   ELSE (100+TO_NUMBER(TO_CHAR(SYSDATE,'YY'))) - TO_NUMBER(SUBSTR(jumin,1,2))
                   END
    END "나이"
FROM users;
-- 2. 모든 사람의 정보를 출력하시오. 단, 이름을 마스킹하시오.
--    마스킹 방법 : 첫글자는 그대로 나머지는 *, *의 갯수는 이름의 길이보다 하나 적다. (예시:abcdef → a*****)
SELECT name 이름, RPAD(SUBSTR(name,1,1),LENGTH(name),'*') "이름(마스킹)" FROM users;

-- 3. 모든 사람의 정보를 출력하시오. 이름 뒤에 다음 문자를 추가하여 출력하시오.
--    1학년 * / 2학년 % / 3학년 # / 4학년 !
...
FROM users;
-- 4. 남자만 출력하시오.
SELECT * FROM users WHERE MOD(SUBSTR(jumin,-1),2)=1;
...
-- LPAD : 지정한 길이만큼 왼쪽부터 지정된 문자로 채운 뒤 리턴한다. : LPAD(값,길이,문자) : 문자를 지정하지 않으면 공백으로 채운다.
-- RPAD : 지정한 길이만큼 오른쪽부터 지정된 문자로 채운 뒤 리턴한다. : RPAD(값,길이,문자) : 문자를 지정하지 않으면 공백으로 채운다.
-- MOD : 나머지를 구하는 함수 : MOD(값1,값2) → 값1을 값2로 나누었을 때의 나머지를 리턴한다.
